...
define([
	"react",
	"machina"
], function(React, machina) {

	var TransitionManager = machina.Fsm.extend({
	...
	var GameBlurb = React.createClass({
		...
		componentWillMount: function() {
			this.transMgr = new TransitionManager({
				values: this.props.trivia
			});
			this.transMgr.on("animation", function(data){
				this.setState({
		...
		componentWillReceiveProps: function(nextProps) {
			console.log(this.props.name + "receiving props")
			if(!nextProps.active) {
				this.transMgr.stop();
			} else {
				this.transMgr.reset();
